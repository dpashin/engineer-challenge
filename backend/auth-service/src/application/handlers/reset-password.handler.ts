  ...
  async execute(command: ResetPasswordCommand): Promise<ResetPasswordResult> {
    ...
    try {
      ...
      if (new Date() > new Date(tokenRecord.expiresAt)) {
        this.logger.warn(`Reset token expired: ${tokenRecord.id}`);
        return {
          success: false,
      ...
      if (tokenRecord.usedAt !== null) {
        this.logger.warn(`Reset token already used: ${tokenRecord.id}`);
        return {
          success: false,
      ...
      if (tokenRecord.revokedAt !== null) {
        this.logger.warn(`Reset token revoked: ${tokenRecord.id}`);
        return {
          success: false,
          error: ResetPasswordError.INVALID_TOKEN,
        };
